...
export class PlanesService {
   ...
   * @returns  Plan creada.
   */
  public getPlan(Plan: any) {
    this.rqManager.setPath('PLAN_ADQUISICIONES_CRUD_SERVICE');
    return this.rqManager.get(`Plan_adquisiciones/` + `${Plan}`);
  }
   ...
   * @returns  Plan creada.
   */
  public crearPlan(Plan: any) {
    this.rqManager.setPath('PLAN_ADQUISICIONES_CRUD_SERVICE');
    return this.rqManager.post(`Plan_adquisiciones/`, Plan);
  }
  /**
   * Put Plan
   *  se modifica un Plan
  ...
  public updatePlan(Plan: any) {
    this.rqManager.setPath('PLAN_ADQUISICIONES_CRUD_SERVICE');
    return this.rqManager.put(`Plan_adquisiciones/`, Plan, Plan.Id);
  }
   ...
   * @returns  Plan creada.
   */
  public getPlanDetallado(Plan: any) {
    this.rqManager.setPath('PLAN_ADQUISICIONES_MID_SERVICE');
    return this.rqManager.get(`RegistrosOrdenadoPorRubro/` + `${Plan}`);
  }
   ...
   * @returns  Plan creada.
   */
  public getVersionesPlan(Plan: any) {
    this.rqManager.setPath('PLAN_ADQUISICIONES_MID_SERVICE');
    return this.rqManager.get(`PlanAdquisicion/versiones/` + `${Plan}`);
  }
   ...
   * @returns  Plan Modificada.
   */
  public publicarPlan(Plan: any) {
    this.rqManager.setPath('PLAN_ADQUISICIONES_MID_SERVICE');
    return this.rqManager.put(`PlanAdquisicion/`, Plan, Plan.Id);
  }
   ...
   * @returns  Plan creada.
   */
  public getVersionPlan(Plan: any) {
    this.rqManager.setPath('PLAN_ADQUISICIONES_CRUD_SERVICE');
    return this.rqManager.get(`Plan_adquisiciones_mongo/` + `${Plan}`);
  }
  ...
  public AjustarDatosPlan(datos: any[]) {
    return datos.map((element: any) => {
      ...
      element.CodigoArka = (
        element['registro_plan_adquisiciones-codigo_arka'] as Array<any>
      ).map((data: any) => {
        return data.Descripcion.split('-')[0];
      });
      if (element.FuenteFinanciamientoId === '') {
        const info = this.ObtenerFuentes(
          element['registro_plan_adquisiciones-actividad']
        );
